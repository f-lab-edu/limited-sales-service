...
public class UserController {
    ...
    public ResponseEntity<String> join(@RequestBody @Validated User user, Errors errors) {
        //validation check
        if(errors.hasErrors()) return new ResponseEntity<>("사용자 정보 입력을 다시 확인해주세요.", HttpStatus.BAD_REQUEST);
    ...
    public String userEmailChk(@RequestBody Map<String,String> userEmail) {
        int cntEmail = userService.checkEmail(String.valueOf(userEmail.get("userEmail")));
        if(cntEmail > 0) {
    ...
    public String userInfoChange(@CurrentUser User user, @RequestBody User reqUser) {
        if(!(reqUser.getUserEmail()).equals(user.getUserEmail())) throw new BadRequestException("사용자 이메일 정보를 다시 확인해주세요.");
    ...
    @ResponseStatus(HttpStatus.OK)
    @DeleteMapping("user")
    public String userInfoDelete(@CurrentUser User user) {
    ...
    public String userChangeAdmin(@CurrentUser User user, @RequestBody Map<String, String> adminCode) {
     ...
     */
    @GetMapping("/re-issue")
    public @ResponseBody ResponseEntity<String> reIssue(@CurrentUser User user, HttpServletResponse response) {
    ...
    @GetMapping("/logout")
    @ResponseStatus(HttpStatus.OK)
    public String logout(@CurrentUser User user, @RequestHeader("Authorization") String authorization, HttpServletRequest request) {
